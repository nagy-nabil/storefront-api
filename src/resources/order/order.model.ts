...
import { ModelBase, OrderState, Order, OrderDoc } from '../../utils/types.js';
export class OrderModel implements ModelBase<Order, OrderDoc> {
    ...
    async createOne(arg: Category): Promise<OrderDoc> {
        try {
            const conn = await client.connect();
            const sql =
    ...
    async updateOne(id: string, arg: Category): Promise<OrderDoc> {
        try {
            const conn = await client.connect();
            const sql =
